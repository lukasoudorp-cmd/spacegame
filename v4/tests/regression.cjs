...
for(const f of ['vendor/d3.min.js','js/world-data.js','js/config.js','js/satellites.js','js/upgrades.js','js/contracts.js','js/save.js','js/diplomacy.js','js/strategy.js','js/events.js','js/map.js'])vm.runInContext(fs.readFileSync(path.join(root,f),'utf8'),c,{filename:f});
...
test('Fresh game starts, saves and schedules exactly one game loop',()=>{
  run('game.init()');assert.equal(run('game.state.money'),100000);assert.equal(run('game.state.satellites.length'),1);assert.equal(run('game.state.satellites[0].activeContract'),null);assert.equal(frames,1);assert.ok(memory.has('orbitalPact_preview4_save_v2'));assert.ok(drawCalls>0);
});
...
test('Legacy saves migrate safely without overwriting the original',()=>{
  memory.delete('orbitalPact_preview4_save_v2');const old={version:1,money:12345,reputation:23,research:5,totalMoneyEarned:150000,playTime:4321,satellites:[{id:0,type:'Scout-1',health:92,activeContract:0}]};memory.set('orbitalPact_preview4_legacy',JSON.stringify(old));run('const migrated=saveSystem.load()');assert.equal(run('migrated.money'),12345);assert.equal(run('migrated.level'),2);assert.equal(run('migrated.satellites[0].health'),92);assert.equal(run('migrated.satellites[0].activeContract'),null);assert.equal(run('migrated.satellites[0].id'),1);assert.equal(memory.get('orbitalPact_preview4_legacy'),JSON.stringify(old));
});
test('Corrupted saves do not crash loading',()=>{
  memory.set('orbitalPact_preview4_save_v2','{bad json');assert.equal(run('saveSystem.load()'),null);memory.set('orbitalPact_preview4_save_v2',JSON.stringify({version:2,money:-50}));assert.equal(run('saveSystem.load()'),null);
});
