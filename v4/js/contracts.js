...
const contractSystem = {
  ...
  quote(state,offer,satellite=null){const efficiency=satellite?satelliteSystem.stats(state,satellite).efficiency:.8;return {reward:Math.round(offer.baseReward*(1+state.upgrades.resolution*.1+state.upgrades.coverage*.03)*(typeof strategySystem!=='undefined'&&strategySystem.has(state,'synthetic-aperture')?1.1:1)),cost:Math.round(offer.baseCost*(1-state.upgrades.communication*.08)),duration:Math.round(offer.baseDuration*(1-state.upgrades.processing*.08)*(.8/efficiency)*(typeof strategySystem!=='undefined'&&strategySystem.has(state,'autonomous-routing')?.88:1))};},
